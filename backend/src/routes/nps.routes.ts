...
async function sendZapiText(phone: string, message: string) {
  ...
  if (!ZAPI_INSTANCE || !ZAPI_TOKEN || !ZAPI_CLIENT_TOKEN) return false
...
export async function npsRoutes(app: FastifyInstance) {
  ...
  app.get('/nps', { preHandler: [requireAuth] }, async (req, reply) => {
    const { patientId, from, to } = req.query as { patientId?: string; from?: string; to?: string }
  ...
  app.get('/nps/summary', { preHandler: [requireAuth] }, async (req, reply) => {
    ...
    if (total === 0) return reply.send({ total: 0, nps: null, promoters: 0, neutrals: 0, detractors: 0, avgScore: null })
  ...
  app.post('/nps/send', { preHandler: [requireAuth] }, async (req, reply) => {
    const { appointmentId } = z.object({ appointmentId: z.string() }).parse(req.body)

    const appt = await prisma.appointment.findUnique({
      where: { id: appointmentId },
      include: { patient: true },
    })
    if (!appt) return reply.status(404).send({ error: 'Consulta não encontrada' })
    if (appt.status !== 'COMPLETED') return reply.status(400).send({ error: 'Consulta não concluída' })

    // Evitar reenvio
    const existing = await prisma.npsResponse.findUnique({ where: { appointmentId } })
    if (existing) return reply.status(409).send({ error: 'NPS já enviado para esta consulta' })

    const clinicName = process.env.NEXT_PUBLIC_CLINIC_NAME ?? 'nossa clínica'
    const message =
      `Olá, ${appt.patient.fullName.split(' ')[0]}! 😊\n` +
    ...
    const npsRecord = await prisma.npsResponse.create({
      data: {
  ...
  app.post('/nps', { preHandler: [requireAuth] }, async (req, reply) => {
    ...
    const record = await prisma.npsResponse.create({
