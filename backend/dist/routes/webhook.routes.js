...
function cleanValue(value, fallback = 'Não informado') {
...
function clinicLocalToUTC(dateStr, timeStr) {
...
function formatClinicHHMM(utcDate) {
...
async function findDoctorsBySpecialty(specialty) {
    ...
    let doctors = await prisma2_1.prisma.doctor.findMany({
    ...
    if (tokens.length === 0)
        return [];
    return prisma2_1.prisma.doctor.findMany({
        where: { OR: tokens.map(token => ({ specialty: { contains: token } })) },
        select,
        orderBy: { specialty: 'asc' },
    });
}
async function getAvailableScheduleForSpecialty(params) {
    const requestedDate = parseISODateOnly(params.date);
    if (!requestedDate)
        return { status: 'error', code: 'INVALID_DATE', message: 'Data inválida. Use YYYY-MM-DD.' };
    const today = parseISODateOnly(getLocalDateISO());
    if (!today)
        return { status: 'error', code: 'SYSTEM_DATE_ERROR', message: 'Erro na data do sistema.' };
    if (requestedDate < today)
        return { status: 'error', code: 'PAST_DATE', message: 'Data no passado. Peça uma nova data.' };
    const doctors = await findDoctorsBySpecialty(params.specialty);
    if (doctors.length === 0)
        return { status: 'success', code: 'NO_DOCTORS_FOUND', slots: [], message: 'Sem médicos cadastrados para essa especialidade.' };
...
async function executeTool(name, args, phone) {
    try {
        ...
        if (name === 'salvar_lead_pre_agendamento') {
            ...
            let lead = await prisma2_1.prisma.lead.findFirst({ where: { phone: leadPhone }, orderBy: { createdAt: 'desc' } });
            if (lead) {
                lead = await prisma2_1.prisma.lead.update({ where: { id: lead.id }, data: { name: finalName, specialty, status: 'PENDING_SCHEDULING' } });
            }
            else {
                lead = await prisma2_1.prisma.lead.create({ data: { name: finalName, phone: leadPhone, specialty, status: 'PENDING_SCHEDULING' } });
            }
        ...
        if (name === 'criar_pre_agendamento') {
            ...
            const doctor = await prisma2_1.prisma.doctor.findUnique({ where: { id: doctorId }, include: { user: { select: { name: true } } } });
            if (!doctor)
                return { status: 'error', message: 'Médico não encontrado.' };
            const lead = await prisma2_1.prisma.lead.findUnique({ where: { id: leadId } });
            if (!lead)
                return { status: 'error', message: 'Lead não encontrado.' };
            // Cria pré-agendamento (registro de rastreamento)
            const preAppt = await prisma2_1.prisma.preAppointment.create({
            ...
            if (patientPhone) {
                ...
                patient = await prisma2_1.prisma.patient.findFirst({ where: { phone: { contains: patDigits } } });
            }
            // 2. Busca por nome exato no cadastro
            if (!patient) {
                patient = await prisma2_1.prisma.patient.findFirst({
            ...
            if (!patient) {
                ...
                patient = await prisma2_1.prisma.patient.create({
            ...
            const appointment = await prisma2_1.prisma.appointment.create({
                data: {
            ...
            await prisma2_1.prisma.lead.update({ where: { id: leadId }, data: { status: 'SCHEDULED' } });
            const doctorName = doctor.user?.name ?? `CRM ${doctor.crm}`;
            return {
                status: 'success',
        ...
        if (name === 'registrar_intencao_atendimento') {
            await prisma2_1.prisma.chatLog.create({ data: { phone, intent: args.intent, message: `Intenção: ${args.intent}`, isUser: false } });
...
// ─── System prompt ────────────────────────────────────────────────────────────
function buildSystemPrompt(config, patientContext, leadContext) {
...
async function callWithTools(messages, phone, depth = 0) {
    if (depth > 5)
        return 'Desculpe, tive dificuldade em processar sua solicitação. Um atendente entrará em contato em breve.';
...
async function processWithAI(phone, userMessage) {
    try {
        ...
        const existingLead = await prisma2_1.prisma.lead.findFirst({
            where: { phone },
        ...
        const history = await prisma2_1.prisma.chatLog.findMany({
        ...
        if (!aiResponse)
            return;
        await prisma2_1.prisma.chatLog.create({ data: { phone, message: aiResponse, isUser: false } });
...
async function webhookRoutes(app) {
    app.post('/webhook', async (request, reply) => {
        ...
        const doctorMatch = await prisma2_1.prisma.doctor.findFirst({
            where: { phone: { contains: phoneDigits } },
            select: { id: true, phone: true },
        });
        if (doctorMatch) {
            const pendingChat = await prisma2_1.prisma.chat.findFirst({
                where: { pendingTransferDoctorId: doctorMatch.id },
                include: { patient: { select: { fullName: true } } },
            });
            if (pendingChat) {
                // Registra a resposta do médico no log
                await prisma2_1.prisma.chatLog.create({ data: { phone, message, isUser: true } });
                if (isAffirmativeResponse(message)) {
                    // Confirma a transferência automaticamente
                    await prisma2_1.prisma.chat.update({
        ...
        let chat = await prisma2_1.prisma.chat.upsert({
            where: { phone },
        ...
        if (!chat.patientId) {
            ...
            if (patient) {
                chat = await prisma2_1.prisma.chat.update({
                    where: { id: chat.id },
