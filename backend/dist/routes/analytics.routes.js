...
async function analyticsRoutes(app) {
    ...
    app.get('/analytics/unit-economics', { preHandler: [requireAuth] }, async (req, reply) => {
        ...
        const refunds = await prisma2_1.prisma.payment.findMany({ where: { status: 'REFUNDED', updatedAt: { gte: fromDate, lte: toDate } } });
        ...
        // ── Churn mensal ──────────────────────────────────────────────────────────
        const cancelledInPeriod = await prisma2_1.prisma.patientEnrollment.count({
            where: { status: 'CANCELLED', cancelledAt: { gte: fromDate, lte: toDate } },
        });
        const activeAtStart = await prisma2_1.prisma.patientEnrollment.count({
        ...
        const activeEnrollmentsByDoctor = await prisma2_1.prisma.appointment.findMany({
            where: { startTime: { gte: fromDate, lte: toDate }, status: { not: 'CANCELLED' } },
            select: { doctorId: true, patientId: true },
        });
        const doctorPatientMap = {};
        for (const a of activeEnrollmentsByDoctor) {
            if (!doctorPatientMap[a.doctorId])
                doctorPatientMap[a.doctorId] = new Set();
        ...
        const totalPatients = await prisma2_1.prisma.patient.count({ where: { deletedAt: null } });
        const completePatients = await prisma2_1.prisma.patient.count({
            where: {
                deletedAt: null,
                cpf: { not: null },
                birthDate: { not: null },
        ...
        for (const e of enrollments) {
            ...
            if (!programMap[key])
                programMap[key] = { nome: e.program.name, count: 0, mrr: 0, patientIds: new Set() };
        ...
        const checkInsTotal = await prisma2_1.prisma.checkIn.count({ where: { scheduledAt: { gte: fromDate, lte: toDate } } });
        const checkInsConcluidos = await prisma2_1.prisma.checkIn.count({
        ...
        const adesaoCheckIns = checkInsTotal > 0
            ...
            : null;
        // ── Pacientes elegíveis ────────────────────────────────────────────────────
        const elegiveisCount = await prisma2_1.prisma.patient.count({
            where: { deletedAt: null, riskProfile: { in: ['METABOLIC', 'CARDIOMETABOLIC', 'HIGH'] } },
        });
        const semProgramaCount = await prisma2_1.prisma.patient.count({
            where: {
                deletedAt: null,
