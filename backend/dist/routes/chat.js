...
async function findDoctorsBySpecialty(specialty) {
    ...
    if (tokens.length === 0)
        return [];
    return await prisma2_1.prisma.doctor.findMany({
...
async function getAvailableScheduleForSpecialty(params) {
    ...
    for (const slot of rawSlots) {
        if (!uniqueSlotsByTime.has(slot.time))
            uniqueSlotsByTime.set(slot.time, slot);
    }
...
function buildSystemPrompt(config, patientContext = '') {
...
const chatRoutes = async (app) => {
    app.post('/chat', async (request, reply) => {
        try {
            ...
            const chat = await prisma2_1.prisma.chat.upsert({
                where: { phone },
            ...
            if (lastMessage && lastMessage.role === 'user' && lastMessage.content) {
                await prisma2_1.prisma.chatLog.create({ data: { phone, message: lastMessage.content, isUser: true } });
            }
            ...
            async function processChatStream(msgs) {
                    ...
                    for (const tool of toolCalls) {
                        ...
                        try {
                            ...
                            if (tool.function.name === 'verificar_agenda') {
                                ...
                                if (lead) {
                                    lead = await prisma2_1.prisma.lead.update({ where: { id: lead.id }, data: { name, specialty, status: 'PENDING_SCHEDULING' } });
                                }
                                else {
                                    lead = await prisma2_1.prisma.lead.create({ data: { name, phone: leadPhone, specialty, status: 'PENDING_SCHEDULING' } });
                                }
                                ...
                                const patient = await prisma2_1.prisma.patient.findFirst({ where: { phone: { contains: patientPhone } } });
                                if (patient) {
                                    const appointments = await prisma2_1.prisma.appointment.count({ where: { patientId: patient.id } });
                                ...
                                const payments = await prisma2_1.prisma.payment.findMany({ where: { patientId: args.patientId }, orderBy: { createdAt: 'desc' } });
                                ...
                                await prisma2_1.prisma.payment.update({ where: { id: args.paymentId }, data: { status: args.status } });
                                result = JSON.stringify({ status: 'success', message: 'Pagamento atualizado.' });
                            }
                            else if (tool.function.name === 'transferir_chat_para_medico') {
                                const chat = await prisma2_1.prisma.chat.upsert({ where: { phone: args.phone }, update: { status: 'TRANSFERRED_TO_DOCTOR', transferredToDoctorId: args.doctorId }, create: { phone: args.phone, status: 'TRANSFERRED_TO_DOCTOR', transferredToDoctorId: args.doctorId } });
                                ...
                                const preAppt = await prisma2_1.prisma.preAppointment.findUnique({ where: { id: args.preAppointmentId } });
                                if (preAppt) {
                                    // Criar appointment real
                                    const doctor = await prisma2_1.prisma.doctor.findUnique({ where: { id: preAppt.doctorId } });
                                    const lead = await prisma2_1.prisma.lead.findUnique({ where: { id: preAppt.leadId } });
                                    if (doctor && lead) {
                                        const startTime = new Date(`${preAppt.date}T${preAppt.time}:00`);
                                        const endTime = new Date(startTime.getTime() + 30 * 60 * 1000); // 30 min
                                        await prisma2_1.prisma.appointment.create({ data: { patientId: lead.patientId || '', doctorId: preAppt.doctorId, startTime, endTime, status: 'SCHEDULED' } });
                                        await prisma2_1.prisma.preAppointment.update({ where: { id: args.preAppointmentId }, data: { status: 'CONFIRMED' } });
                ...
                if (assistantContent.trim()) {
                    await prisma2_1.prisma.chatLog.create({ data: { phone, message: assistantContent, isUser: false } });
                }
                reply.raw.end();
            }
