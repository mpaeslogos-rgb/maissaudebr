...
exports.gerarXmlTiss = gerarXmlTiss;
...
// ─── Gerador principal ────────────────────────────────────────────────────────
function gerarXmlTiss(lote, sequencial = 1) {
    const now = new Date();
    const dataHoje = fmtDate(now.toISOString());
    const hora = now.toTimeString().substring(0, 8);
    const guiasXml = [
        ...lote.guiasConsulta.map(g => guiaConsultaXml(g).replace(/<!-- ans:registroANS -->/g, lote.operadora.registroANS)),
        ...lote.guiasSPSADT.map(g => guiaSPSADTXml(g).replace(/<!-- ans:registroANS -->/g, lote.operadora.registroANS)),
    ].join("\n");
    const prestadorPart = `
  <ans:prestadorParaOperadora>
    ...
    const hash = (0, node_crypto_1.createHash)("md5").update(prestadorPart).digest("hex");
