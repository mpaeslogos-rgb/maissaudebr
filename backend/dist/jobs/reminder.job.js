...
function getDateRangeUTC(daysFromNow) {
    const now = new Date();
...
async function sendWhatsApp(phone, message) {
    ...
    try {
        await axios_1.default.post(`${url}/message/sendText/${instance}`, { number: normalized, text: message }, { headers: { apikey: key, 'Content-Type': 'application/json' } });
        return true;
    }
    catch {
        return false;
...
async function runReminders(isToday) {
...
function startReminderJobs() {
    ...
    node_cron_1.default.schedule('0 8 * * *', () => runReminders(false), { timezone: 'America/Manaus' });
    // D-0: dispara às 7h no dia da consulta
    node_cron_1.default.schedule('0 7 * * *', () => runReminders(true), { timezone: 'America/Manaus' });
