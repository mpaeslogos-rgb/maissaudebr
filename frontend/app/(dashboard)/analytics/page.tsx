...
import { TrendingUp, TrendingDown, Users, DollarSign, Star, Activity } from "lucide-react";
...
interface UnitEconomics {
  ...
  assinaturas: { mrr: number; totalAtivas: number; programas: { nome: string; count: number }[] };
...
function KpiCard({ label, value, sub, icon: Icon, trend }: { label: string; value: string; sub?: string; icon: React.ElementType; trend?: "up" | "down" | null }) {
  return (
...
export default function AnalyticsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"kpis" | "dre" | "mix">("kpis");
        ...
        <>
          ...
          <div className="flex border-b border-gray-200 mb-6">
            {(["kpis", "dre", "mix"] as const).map(t => (
              <button key={t} onClick={() => setActiveTab(t)}
                ...
                {t === "kpis" ? "Unit Economics" : t === "dre" ? "DRE" : "Mix de Receita"}
              </button>
            ))}
          </div>
          ...
          {activeTab === "kpis" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-5">
                <h3 className="font-semibold text-gray-900 mb-4">Atendimentos</h3>
              ...
              {data.assinaturas.programas.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                  <h3 className="font-semibold text-gray-900 mb-4">Matrículas por Programa</h3>
                      ...
                      <div key={p.nome} className="flex items-center gap-3">
                        <div className="flex-1">
                          ...
                          <div className="bg-gray-100 rounded-full h-2">
                            <div className="bg-primary-500 h-2 rounded-full" style={{ width: `${(p.count / data.assinaturas.totalAtivas) * 100}%` }} />
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {catData.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                  <h3 className="font-semibold text-gray-900 mb-4">Top Despesas</h3>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={catData} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis type="number" tickFormatter={v => `R$${(v/1000).toFixed(0)}k`} tick={{ fontSize: 10 }} />
                      <YAxis type="category" dataKey="cat" width={100} tick={{ fontSize: 10 }} />
                      <Tooltip formatter={(v: number) => fmt(v)} />
